...
bunch.Packager = function( ) {
	...
	publicFunc = {
		...
		* @return nothing
		*/
		'logError': function( bundleName, errName, errMsg ) {
			var errCode;
			if ( privateFunc.fileType( path.extname( bundleName ) ) === 'css' ) {
				errCode = 'body:before {\n' + 
					'content: "Bunch.js Error - ' + errName + ': ' + errMsg + '";\n' + 
					'background: #333;\n' + 
					'color: #fff;\n' +
	...
	privateFunc = {
		...
		'readBundledFile': function( filePath, callback ) {
			...
			path.exists( fullPath, function( exists ) {
				if ( exists ) {
					...
					throw new Error( 'FileNotFound: Could not find file ' + fullPath + ' : ' + filePath + ' : ' +  privateFunc.getDir( path.extname( filePath ), 'source' ) );
				}
			} );
		},
		/**
		...
		*/
		'prependToBundle': function( bundleName, content, callback ) {
			var fullPath = path.join( privateFunc.getDir( path.extname( bundleName ), 'build' ), bundleName ),
				reporter = settings.reporter;
